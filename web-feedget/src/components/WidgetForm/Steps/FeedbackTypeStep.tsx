import { FeedbackType, feedbackTypes } from '..';
import { CloseWidgetButton } from '../../CloseButton';

interface FeedbackTypeStepProps {
  handleSelectType: (type: FeedbackType) => void;
}

export function FeedbackTypeStep({ handleSelectType }: FeedbackTypeStepProps) {
  return (
    <>
      <header>
        <span className="text-xl leading-6">Deixe seu feedback</span>
        <CloseWidgetButton />
      </header>
      <div className="flex py-8 gap-2 w-full">
        {Object.entries(feedbackTypes).map(([key, { title, image }]) => (
          <button
            key={key}
            type="button"
            className="bg-zinc-800 rounded-lg py-5 w-24 flex-1 flex flex-col items-center justify-center gap-2 border-2 border-transparent hover:border-brand-500 focus:border-brand-500 focus:outline-none"
            onClick={() => handleSelectType(key as FeedbackType)}
          >
            <img src={image.source} alt={image.alt} />
            <span>{title}</span>
          </button>
        ))}
      </div>
    </>
  );
}
